...
void MigrationHelper::initialize(Module *module, TypeGraph *tg) {
...
string MigrationHelper::getType(Instruction &inst) {
  auto type = inst.getType();
  if (auto *load = dyn_cast<LoadInst>(&inst)) { type = load->getType(); }
  if (auto *store = dyn_cast<StoreInst>(&inst)) {
  ...
  if (auto *alloca = dyn_cast<AllocaInst>(&inst)) {
    ...
    if (!tyHelper.isOpaque(type)) return tyHelper.getReference(type);
  }
...
void DebugInfoHelper::initialize(Module *module, TypeGraph *tg) {
      ...
      // struct
        ...
        if (composite->getTag() == dwarf::DW_TAG_structure_type) {
          if (composite->getName() == structName) {
            // empty elements, skip
            if (composite->getElements().empty()) continue;
  ...
  for (auto &global : module->globals()) {
    ...
    if (di_global_exps.empty()) { continue; }
...
void DebugInfoHelper::parseDILocalVar(
    Instruction &inst, map<Value *, vector<DILocalVariable *>> &diLocalMap) {
  // check if is call inst
  if (auto *call = dyn_cast<CallInst>(&inst)) {
    auto call_func = call->getCalledFunction();
    if (!call_func) return;
    if (call_func->arg_size() < 2) return;
...
bool DebugInfoHelper::hasDebugInfo(Module &M) {
  for (auto &F : M)
    for (auto &BB : F)
      for (auto &I : BB)
        if (auto DILoc = I.getDebugLoc()) return true;
  return false;
}
  ...
  switch (tag) {
    ...
    case dwarf::DW_TAG_array_type: {
      ...
      for (auto element : elements) {
        if (auto *sub = dyn_cast<DISubrange>(element)) subrangeCount++;
      }
      name = basename + string(subrangeCount, '*');  // multi-dimension array
    } break;
...
void TBAAHelper::initialize(Module *module, TypeGraph *tg) {
  for (auto &func : *module) {
    for (auto &basic_block : func) {
      for (auto &inst : basic_block) {
        // handle TBAA
...
bool TBAAHelper::isScalarType(string &type) {
  return type == "i1" || type == "i8" || type == "i16" || type == "i32" ||
         type == "i64" || type == "float" || type == "double";
}

string TBAAHelper::getTBAAType(MDNode *tbaa, Function &func,
                               Instruction &inst) {
  // get first field
...
string TBAAHelper::getTypeName(MDNode *tbaaType) {
  ...
  if (isOmnipotentChar(accessTy)) { return baseTyName->getString().str(); }

  return getTypeName(accessTy);
}

// parse TBAA type name
string TBAAHelper::parseTypeName(MDNode *tbaa) {
  auto *baseTy = dyn_cast<MDNode>(tbaa->getOperand(0));
  auto *accessTy = dyn_cast<MDNode>(tbaa->getOperand(1));

  if (isOmnipotentChar(accessTy)) return "";

  auto name = getTypeName(accessTy);
  if (name == "any pointer") return "";

  return name + "*";
}

bool TBAAHelper::isOmnipotentChar(MDNode *tbaa) {
  auto *accessTyName = dyn_cast<MDString>(tbaa->getOperand(0));
  return accessTyName->getString() == "omnipotent char";
}

void CombHelper::initialize(Module *module, TypeGraph *tg) {
  ...
  for (auto &global : module->globals()) {
    ...
    for (auto di_global_exp : di_global_exps) {
      auto di_global = di_global_exp->getVariable();
      auto di_type_name = diHelper->getDITypeName(di_global->getType()) + "*";
      tg->put(nullptr, &global, di_to_ir_type(di_type_name));
    }
  }
  ...
  for (auto &func : *module) {
    for (auto &basic_block : func) {
      for (auto &inst : basic_block) {
        ...
        if (diLocalMap.find(value) != diLocalMap.end()) {
          for (auto di_local : diLocalMap[value]) {
            auto di_type_name = diHelper->getDITypeName(di_local->getType());
...
bool CombHelper::hasDebugInfo(Module &M) {
