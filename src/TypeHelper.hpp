...
class TypeHelper {
 ...
 public:
  ...
  bool isOpaque(string &type) {
    return type == "ptr";
  }
  bool isOpaque(set<string> &typeset) {
    return typeset.count("ptr");
  }
  bool isOpaque(Type *type) {
    return getTypeName(type) == "ptr";
  }
  ...
  void count(Module *module, TypeGraph *tg) {
    ...
    for (auto &global : module->globals()) {
      total++;
      if (tg->isOpaque(nullptr, &global)) { opaque++; }
    }
    ...
    for (auto &func : *module) {
      for (auto &basic_block : func) {
        for (auto &inst : basic_block) {
          ...
          total++;
          if (tg->isOpaque(&func, &inst)) { opaque++; }
        }
      }
    }
  ...
  void dumpOpaque(Module *module, TypeGraph *tg) {
    ...
    for (auto &global : module->globals()) {
      ...
      cnt++;
      if (!tg->get(nullptr, v)) { errs() << "[MISS] " << *v << "\n"; }
    }
