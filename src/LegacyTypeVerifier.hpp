...
class LegacyTypeVerifier : public PassInfoMixin<LegacyTypeVerifier> {
 private:
  ...
  bool match(string &ir_type, string &di_type) {
    if (ir_type == di_type) { return true; }

    if (ir_type == "i8" && di_type == "i1") { return true; }

    if (ir_type == "i64" && di_type == "i32") { return true; }
      ...
      for (auto &BB : F) {
        for (auto &inst : BB) {
          ...
          if (auto *call = dyn_cast<CallInst>(&inst)) {
            ...
            if (!call_func) continue;
            if (call_func->arg_size() < 2) continue;
            ...
            if (call_func && call_func->isIntrinsic() &&
              ...
              if (auto arg = call->getArgOperand(1)) {
                ...
                auto di_value = dyn_cast<DILocalVariable>(metadata);

                total_cnt++;
                string ir_type = tyHelper->getTypeName(real_val->getType());
                string di_type = diHelper->getDITypeName(di_value->getType());
