...
def mlta(branch, mlta_dir, target):
    cmd = f"git checkout {branch} && make clean && make && ./build/lib/kanalyzer {target}"
...
if __name__ == "__main__":
    mlta_dir = os.path.join(os.path.pardir, os.path.pardir, "mlta")
    bc_dir = os.path.join("~/type-analysis/TypeCopilot", "bc")
    table = []

    # baseline
    _, baseline_avg = mlta("baseline", mlta_dir, os.path.join(bc_dir, "linux.bc"))
    print("[DEBUG] baseline_avg:")
    print(baseline_avg)

    # modified
    modified_callsite, modified_callee = mlta("op", mlta_dir, os.path.join(bc_dir, "linux.bc"))
    print("[DEBUG] modified_callsite:")
    print(modified_callsite)
    print("[DEBUG] modified_callee:")
    print(modified_callee)
    print(modified_callee / modified_callsite)
    ...
    # original
    original_callsite, original_callee = mlta("main", mlta_dir, os.path.join(bc_dir, "llvm14-linux.bc"))
